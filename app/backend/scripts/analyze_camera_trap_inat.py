"""Analyze camera trap images for species using iNaturalist EVA02 model.

This script uses the EVA02 model fine-tuned on iNaturalist 2021 dataset,
which covers 10,000 species with 92% accuracy. It performs particularly
well on European bird species including turtle doves.
"""
...
from services.camera_trap_inat import (
...
def write_csv_results(results: list[ImageResult], output_path: Path) -> None:
    fieldnames = [
        ...
        "taxonomic_level", "flagged_for_review", "review_reason", "prediction_source",
...
def write_summary(results: list[ImageResult], output_path: Path) -> None:
    ...
    summary["model"] = "iNaturalist-EVA02"
...
def print_summary(results: list[ImageResult]) -> None:
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("ANALYSIS SUMMARY (iNaturalist EVA02)")
    logger.info("=" * 60)
...
def main():
    ...
    logger.info(f"Using model: iNaturalist EVA02 (10,000 species)")
    ...
    write_json_results(results, output_dir / f"results_inat_{timestamp}.json")
    if not args.json_only:
        write_csv_results(results, output_dir / f"results_inat_{timestamp}.csv")
    write_summary(results, output_dir / f"summary_inat_{timestamp}.json")
